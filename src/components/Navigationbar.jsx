...
const Navbar = () => {
  ...
  const handleDownload = () => {
    setIsLoading(true);
    const cvFile = '/AYAK MANNA CV.pdf'; 
    const link = document.createElement('a');
    link.href = cvFile;
    link.download = 'AYAK MANNA CV.pdf'; 
        ...
        <div className="flex items-center justify-between h-20">
          <div className="flex items-center">
            <Link to="/" className="flex items-center gap-1" onClick={() => handleNavClick({ name: "Home", href: "/#hero" })} aria-label="Home">
              <img src={logo} alt="AM Logo" className="w-16 h-16" loading="lazy" />
              <span className="text-white text-3xl font-semibold">Ayak Manna</span>
            </Link>
          </div>
